...
If I wanted to spend more time with this, I might consider an appropriate solution involving C++ *Boost.Units* in order to better capture the **Currency**, in this case *U.K. currency*, as well as the units, *Pence* or *Pounds*. However, for simplicity and early delivery, I find it easier to at least prototype the approach using the tried and true tools of .NET and C#, at least in my experience. In so many words, when the chips are down, I like to play to my strengths.

When using a solver, in most cases, one may be able to estimate, but does not necessarily know, the number of possible solutions; nor would one want to necessarily perform either of these activities. That's what the problem solver is there to furnish, if possible, if necessary. One may solve for any number of solutions, or all of them, etc, depending on the constraints and any optimization functions that were provided; let's say I wanted to maximize the responses involving *10p*, then I would receive the solution or solutions that maximized this result from the problem description.

For purposes of solving the problem at hand, however, I just wanted to collate all of the responses and report the total number of possible responses, as well as print those out in the report.
